...
def vital_sign_summary_statistics(data_analysis_folder, vital_sign):
    ...
    colors = sns.color_palette("deep")
    # BA plot
    sm.graphics.mean_diff_plot(camera, truth, ax=axs[0, 0], scatter_kwds={"color":colors[0]})
    ...
    axs[1, 0].scatter(truth, camera, color=colors[0])
    reg = LinearRegression().fit(camera.reshape(-1, 1), truth)
    ...
    if "Tidal" not in vital_sign:  # can use integers for all other measurements
        ...
        while not threshold_reached:
            cp = coverage_probability_abs(truth, camera, kappas[i]) * 100
    ...
    axs[1, 1].plot(kappas, coverage_probability, color=colors[0])
