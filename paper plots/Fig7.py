...
(
    ...
    data["ECG timestamps"],  # Timestamps corresponding to the ECG data
    data["ECG rate"],  # ECG rate data
    data["POS kalman fourier"],  # Processed POS signal data
    data["CHROM kalman fourier"],
    data["POS kalman peaks"],
    data["CHROM kalman peaks"],
    data["Timestamps"],  #
    data["Average"],
)
...
axs[0, 0].plot(ECG_timestamps, ECG_rate, label="ECG", color=colors[0])
axs[0, 0].plot(timestamps, POS_kalman_peaks, label="Peak POS", color=colors[1])
axs[0, 0].plot(
    timestamps,
    CHROM_kalman_peaks,
    label="Peak CHROM",
...
axs[0, 0].set_ylabel("Heart rate (bpm)", fontsize=8)
axs[0, 0].set_xlabel("Time (s)", fontsize=8)
axs[0, 0].set_ylim(100, 200)
axs[0, 0].set_title("(a) Peak counting", fontsize=8)
axs[0, 0].legend(loc="lower center", fontsize=8, frameon=False)
...
axs[1, 0].plot(ECG_timestamps, ECG_rate, label="ECG", color=colors[0])
axs[1, 0].plot(timestamps, POS_kalman, label="Fourier POS", color=colors[1])
axs[1, 0].plot(timestamps, CHROM_kalman, label="Fourier CHROM", color=colors[2])
axs[1, 0].plot(timestamps, average, label="Average", color=colors[3])
axs[1, 0].set_ylabel("Heart rate (bpm)", fontsize=8)
axs[1, 0].set_xlabel("Time (s)", fontsize=8)
axs[1, 0].set_ylim(100, 200)
axs[1, 0].set_title("(b) Fourier analysis", fontsize=8)
axs[1, 0].legend(loc="lower center", fontsize=8, frameon=False)
...
axs[0, 1].set_ylabel("Difference (bpm)", fontsize=8)
axs[0, 1].set_xlabel("Means (bpm)", fontsize=8)
axs[0, 1].set_title("(c) Heart rate BA", fontsize=8)
...
axs[0, 2].set_xlabel("Camera - Ground Truth (bpm)", fontsize=8)
axs[0, 2].set_ylabel("Frequency", fontsize=8)
axs[0, 2].set_title("(e) Heart rate errors", fontsize=8)
...
axs[1, 1].set_ylabel("Camera (bpm)", fontsize=8)
axs[1, 1].set_xlabel("Ground Truth (bpm)", fontsize=8)
axs[1, 1].set_title("(d) Heart rate correlation", fontsize=8)
...
axs[1, 2].set_xlabel("Absolute error (bpm)", fontsize=8)
axs[1, 2].set_ylabel("Coverage probability (%)", fontsize=8)
axs[1, 2].set_title("(f) Heart rate CP", fontsize=8)
